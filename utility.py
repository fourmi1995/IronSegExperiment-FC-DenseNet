...
def _parse_data_infer_label(image_paths):
    image_content = tf.read_file(image_paths)
...
def _normalize_data_infer_label(image):
    image = tf.cast(image, tf.float32)
...
def _parse_data(image_paths, mask_paths):
    ...
    image_content = tf.read_file(image_paths)
    images = tf.image.decode_png(image_content, channels=3)
    images = tf.image.resize_images(images,(256,256),method=0)

    mask_content = tf.read_file(mask_paths)
    masks = tf.image.decode_png(mask_content, channels=1)
    masks = tf.image.resize_images(masks,(256,256),method=0)

    return images, masks


def data_batch(image_paths, mask_paths, batch_size=4, augment=False, num_threads=2,train_flag=False):
    ...
    if  mask_paths and train_flag:
        mask_name_tensor = tf.constant(mask_paths)
        data = tf.data.Dataset.from_tensor_slices(
            (images_name_tensor, mask_name_tensor))
        data = data.map(
            _parse_data, num_parallel_calls=num_threads).prefetch(100)
        data = data.map(
            _resize_data, num_parallel_calls=num_threads).prefetch(100)
        data = data.map(_normalize_data,
                        num_parallel_calls=num_threads).prefetch(100)
        data = data.batch(batch_size)
        data = data.shuffle(100)
        print('11111111111111111111111')
    elif not(mask_paths) and not train_flag:
    
        data = tf.data.Dataset.from_tensor_slices((images_name_tensor))
        data = data.map(_parse_data_infer_label,
                        num_parallel_calls=num_threads).prefetch(100)
        data = data.map(_resize_data_infer,
                        num_parallel_calls=num_threads).prefetch(100)
        data = data.map(_normalize_data_infer_label,
                        num_parallel_calls=num_threads).prefetch(100)
        data = data.batch(batch_size)
        print('22222222222222222222222222')
    elif not(mask_paths) and train_flag:
        data = tf.data.Dataset.from_tensor_slices((images_name_tensor))
