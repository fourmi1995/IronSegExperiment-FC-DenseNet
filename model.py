...
class DenseTiramisu(object):
    ...
    def infer(self, image_dir,mask_dir, batch_size, ckpt, output_folder):
        """
        ...
        mask_paths = [os.path.join(mask_dir, x) for x in os.listdir(mask_dir) if x.endswith('.png') or x.endswith('.jpg')]
        #print('1111111111111111',os.listdir(image_dir))
        infer_data, infer_queue_init = utility.data_batch(
            image_paths,False, batch_size,train_flag=True)
        infer_mask, infer_queue_mask_init = utility.data_batch(
            mask_paths,False, batch_size,train_flag=False)            
        image_ph = tf.placeholder(tf.float32, shape=[None, 256, 256, 3])
        mask_ph = tf.placeholder(tf.float32, shape=[None, 256, 256, 1])
        training = tf.placeholder(tf.bool, shape=[])
        ...
        with tf.Session() as sess:
            ...
            for _ in range(len(image_paths) // batch_size):
                ...
                for j in range(prediction.shape[0]):
                    cv2.imwrite(os.path.join(output_folder, '{}.png'.format(j)),  45*prediction[j, :, :])
                    cv2.imwrite(os.path.join(output_folder, 'label'+'{}.png'.format(j)),  45*label[j, :, :])
                    cv2.imwrite(os.path.join(output_folder, 'original'+'{}.png'.format(j)),  255*image[j, :, :])
